...
#define WLAN_SSID       "WIFI NAME"                            //Enter the WiFi name
#define WLAN_PASS       "WIFI PASSWORD"                        //Enter the WiFi password
  ...
  while ((subscription = mqtt.readSubscription(5000))) {
    if (subscription == &voice_control) {     
      ...
      if(feed[0] == 'd'){
        ...
        ax12a.begin(BaudRate, DirectionPin, &Serial);
        delay(100);
        ax12a.moveSpeed(ID, bitPosition, 1023);
        Serial.flush();
...
// Input argument: string that starts with a caracter followed by a number between 0 and 999
//
int calculate_position(String feed){  
  int bitPosition = 0;
  int anglePosition = 0;
